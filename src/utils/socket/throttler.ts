...
import { IClientToServerEvents, IInterServerEvents, IServerToClientEvents, ISocketData } from '@/constant/socket';
...
const canBeServed = (
    ...
    if (previous) {
        ...
        if (diff < TIME_LIMIT) {
            ...
            setTimeout(() => {
                logger.info(`Socket:: IP: ${clientIp} Packet: [${packet.toString()}] NSP: ${socket.nsp.name} Disconnected, reason: TOO_MANY_EMITS`);
                logger.error(`Socket:: connection kicked due to too many emits in a short time. IP: ${clientIp}`);
