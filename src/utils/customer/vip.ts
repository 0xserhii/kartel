...
const { numLevels, minWager, maxWager, rakeback, vipLevelNAME, vipLevelCOLORS } = CCustomer_Vip_Config;
...
const generateVIPLevels = (
    ...
    for (let i = 0; i < numLevels; i++) {
        const level = {
            name: (i + 1).toString(),
            wagerNeeded: parseFloat((minWager + (maxWager - minWager) * Math.pow(i / numLevels, 2)).toFixed(2)),
            rakebackPercentage: parseFloat((rakeback / (1 + Math.exp(-5 * (i / numLevels - 0.5)))).toFixed(2)),
...
const checkAndApplyRakeback = async (userId: string, houseRake: number): Promise<void> => {
