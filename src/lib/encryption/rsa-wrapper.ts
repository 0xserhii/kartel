...
export default class RSAWrapper {
    ...
    async publicEncrypt(keyInPemFormat: string, message: string): Promise<string> {
        const key = await this.importPublicKey(keyInPemFormat);
        const encrypted = await crypto.encrypt(rsaParams, key, this.converterWrapper.str2abUtf8(message));
        return this.converterWrapper.arrayBufferToBase64String(encrypted);
    }

    async privateDecrypt(keyInPemFormat: string, encryptedBase64Message: string): Promise<string> {
        const key = await this.importPrivateKey(keyInPemFormat);
        const decrypted = await crypto.decrypt(rsaParams, key, this.converterWrapper.base64StringToArrayBuffer(encryptedBase64Message));
