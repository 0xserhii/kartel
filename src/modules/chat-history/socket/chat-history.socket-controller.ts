...
class ChatHistorySocketHandler {
  ...
  public authHandler = async (token: string) => {
    if (!token) {
      this.loggedIn = false;
      this.user = null;
      return this.socket.emit('error', 'No authentication token provided, authorization declined');
    }
  ...
  public sendMessageHandler = async (message: string) => {
    try {
      if (!this.loggedIn) return this.socket.emit('notify-error', `You are not logged in!`);
      ...
      const userdata: TChatUser = {
        ...
        hasVerifiedAccount: this.user?.hasVerifiedAccount!,
      ...
      return this.socket.emit('notify-error', `An error is occured on save chat history!`);
    }
  };
  ...
  public getChatHistoryHandler = async (sentAt: Date) => {
    ...
    if (sentAt) {
      previousChatHistory = await this.chatHistoryService.fetchEarlierChatHistories(sentAt, 12);
    } else {
      previousChatHistory = await this.chatHistoryService.fetchEarlierChatHistories(new Date(), 12);
    }
    if (previousChatHistory?.length > 0) {
      ...
      this.socket.emit('notify-error', 'Error ocurred when fetched previous chat histories!');
    }
  };
}
