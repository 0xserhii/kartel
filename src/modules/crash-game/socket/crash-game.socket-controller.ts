...
import { ICrashGameModel, IUpdateParams, TAutoCrashBetPayload, TJoinGamePayload } from "../crash-game.interface";
...
export class CrashGameSocketController {
    ...
    public initializeSubscribe = async () => {
        const players = await _.map(CrashGameSocketController.gameStatus.players, (p) => CrashGameSocketController.formatPlayerBet(p));
        this.socket.emit('game-status', { players, gameStatus: CrashGameSocketController.gameStatus });
    }
    ...
    public autoCrashBetHandler = async (
        data: TAutoCrashBetPayload,
    ) => {
        if (!this.loggedIn) {
        ...
        try {
            ...
            const bet = await this.autoCrashBetService.getItem({ user: this.user._id, status: true });
    ...
    public joinGameHandler = async (data: TJoinGamePayload, emitPlayersBet: () => void) => {
        ...
        if (
            ...
            return this.socket.emit("game-join-error", "Game is currently in progress!");
        }
        ...
        try {
            ...
            if (
                ...
                delete CrashGameSocketController.gameStatus.pending[userId];
                CrashGameSocketController.gameStatus.pendingCount--;
                return this.socket.emit("game-join-error", "You can't afford this bet!");
            }
    ...
    public betCashoutHandler = async (doCashOut: (bet: string, at: number, forced: boolean, callback: (err: any, result: any) => void) => void) => {
        if (!this.loggedIn) {
            return this.socket.emit("bet-cashout-error", "You are not logged in!");
        }
        ...
        if (
            CrashGameSocketController.gameStatus.status !==
            CGAME_STATES.InProgress
        ) {
            return this.socket.emit("bet-cashout-error", "There is no game in progress!");
        }

        // Calculate the current multiplier
        const elapsed =
            Date.now() -
            CrashGameSocketController.gameStatus.startedAt!.getTime();
        let at = this.growthFunc(elapsed);

        // Check if cashout is over 1x
        if (at < 101) {
            return this.socket.emit("bet-cashout-error", "The minimum cashout is 1.01x!");
        }
        ...
        if (at > CrashGameSocketController.gameStatus.crashPoint!) {
            return this.socket.emit("bet-cashout-error", "The game has already ended!");
        }

        // Check if user already cashed out
        if (bet.status !== this.betStatus.Playing) {
            return this.socket.emit("bet-cashout-error", "You have already cashed out!");
        }
