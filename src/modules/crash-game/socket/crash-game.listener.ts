...
import { CBET_STATES, CCrash_Config, CGAME_STATES, CTime, ECrashGameEvents } from "../crash-game.constant";
import {
    IUpdateParams,
...
import { generateCrashRandom, generatePrivateSeedHashPair } from "@/utils/crypto/random";
...
class CrashGameSocketListener {
    ...
    private logoPrefix: string = "[Crash Game Socket]::: ";
    // Services
    ...
    private blockGame = () => {
        ...
        const loop = (): NodeJS.Timeout => {
            const ids: string[] = Object.keys(CrashGameSocketController.gameStatus.pending);
    ...
    private startGame = async () => {
        try {
            ...
            CrashGameSocketController.gameStatus.duration = Math.ceil(
                16666.666667 * Math.log(0.01 * (CrashGameSocketController.gameStatus.crashPoint + 1))
            );
            CrashGameSocketController.gameStatus.startedAt = new Date();
            ...
            // Updating in db
            await this.crashGameService.updateById(CrashGameSocketController.gameStatus._id as any, {
    ...
    public runTick = () => {
        ...
        const elapsed = Date.now() - CrashGameSocketController.gameStatus.startedAt!.getTime();
    ...
    public doCashOut = async (
        ...
        // Check if bet is still active
        if (CrashGameSocketController.gameStatus.players[playerID]?.status !== CBET_STATES.Playing) {
            return;
        }

        // Update player state
        CrashGameSocketController.gameStatus.players[playerID].status = CBET_STATES.CashedOut;
        CrashGameSocketController.gameStatus.players[playerID].stoppedAt = elapsed;

        if (forced) {
            CrashGameSocketController.gameStatus.players[playerID].forcedCashout = true;
        }
        ...
        CrashGameSocketController.gameStatus.players[playerID].winningAmount = winningAmount;

        if (cb) {
            cb(null, CrashGameSocketController.gameStatus.players[playerID]);
        }

        const { status, stoppedAt } = CrashGameSocketController.gameStatus.players[playerID];
        ...
        // Updating in db
        const updateParam = { $set: {} };
        updateParam.$set["players." + playerID] = CrashGameSocketController.gameStatus.players[playerID];
        await this.crashGameService.updateById(
            CrashGameSocketController.gameStatus._id as any,
            updateParam
        );
    ...
    public endGame = async () => {
        logger.info(
            this.logoPrefix + `Ending game at ${CrashGameSocketController.gameStatus.crashPoint! / 100}`
        );
