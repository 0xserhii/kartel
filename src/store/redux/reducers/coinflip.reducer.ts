...
export default function coinflipReducer(state = initialState, action): ICoinflipState {
    switch (action.type) {
        case ECoinflipSocketAction.RECEIVE_MSG:
            return { ...state, msg: action.payload };

        case ECoinflipSocketAction.COINFLIPGAME_ROLLED:
            return { ...state, gameData: { ...state.gameData, ...action.payload as ICoinflipData }, gameStatus: true };
