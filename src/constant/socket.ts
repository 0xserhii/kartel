...
export interface IClientToServerEvents {
    ...
    //minesgame events
    'create-new-minesgame': (data: { betAmount: number; denom: string; betMinesCount: number }) => void;
...
export interface IServerToClientEvents {
    ...
    //crashgame Events
    'game-status': (data: { players: TFormattedPlayerBetType[]; game_status: number }) => void;
    ...
    'minesgame-ended': (data: { winAmount: number | null; mines: number[] }) => void;

    //chat
    message: (data: { _id: Types.ObjectId; user: TChatUser; message: string; sentAt: Date }) => void;
